...
reg add "\\%MACHINENAME%\HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 0 /f >nul 2>&1

if %ERRORLEVEL%==0 (
	echo.
	echo.
	call :cecho 0a "Enabling TSConnections Succeeded"
) else (
	echo.
	echo.
	call :cecho 0c "Enabling TSConnections Failed. Make sure " /nb
...
cmdkey /generic:%MACHINENAME% /user:%MACHINENAME%\administrator >nul 2>&1
::ping 127.0.0.1 -n 15>nul 2>&1
echo.
call :cecho 0c *************************************
call :cecho 0c *************************************
call :cecho 0c "***WINDOW WILL CLOSE AUTOMATICALLY***"
call :cecho 0c "*** AFTER REMOTE SESSION. DO NOT  ***"
call :cecho 0c "***        CLOSE WINDOW!          ***"
call :cecho 0c *************************************
call :cecho 0c *************************************

mstsc /span /v:%MACHINENAME%

::cmdkey /delete:%MACHINENAME% >nul 2>&1
for /f "tokens=2 delims==" %%c in ('cmdkey /list ^| findstr /i %MACHINENAME%') do (cmdkey /delete %%c)
::reg add "\\%MACHINENAME%\HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 1 /f >nul 2>&1
::
::if %ERRORLEVEL%==0 (
::	echo.
::	echo.
::	echo Closing Remote Desktop Connection...
::	ping 127.0.0.1 >nul 2>&1
::	echo.
::	echo.
::	call :cecho 0a "Disabling TSConnections Succeeded"
::	echo.
::	echo.
::	echo Closing...
::	ping 127.0.0.1 -n 10 >nul 2>&1
...
::	echo.
::	echo.
::	call :cecho 0c "Disabling TSConnections Failed. Please make sure to manually disable TSConnections."
::	pause
::)
goto:eof
